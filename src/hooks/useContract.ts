...
export function useGovernanceContract(): Contract | null {
  ...
  const latestAlphaGovernanceAddress =
    activeProtocol?.governanceAlphaAddresses[
      activeProtocol?.governanceAlphaAddresses.length - 1
    ];
  ...
  return useContract(
    ...
    activeProtocol?.id === AAVE_GOVERNANCE.id
      ? GOVERNANCE_AAVE_ABI
      : GOVERNANCE_ABI,
    true
  );
...
export function useGovTokenContract(): Contract | null {
  ...
  return useContract(
    govToken ? govToken.address : undefined,
    isAave ? AAVE_ABI : UNI_ABI,
    true
  );
}
