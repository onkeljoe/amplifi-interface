  ...
  switch (settings.source) {
    case "excel":
      if (!settings.excelSheetName) {
        console.error(
          "Unable to fetch excel sheet, sheet name must be the sheet TAB name"
        );
        return;
      }
      if (settings.type === "airdrop") {
        const res = await fetchListExcel(
          settings.idOrUrl,
          settings.excelSheetName,
          settings.type,
          undefined
        );
        return {
        ...
        return {
          data: [
            convertExcelResponseToAmpPayoutBasicBoostList(res),
            convertExcelResponseToAmpPayoutBoostedBonusList(res),
          ],
          type: settings.type,
        };
      }
...
function convertExcelResponseToAmpPayoutBasicBoostList(
  ...
  excelRes.table.rows.forEach((r) => {
    ...
    _airdropList = {
      ...
      [r.c[0].v]:
        ...
        r &&
        r.c &&
        r.c[basicBoost2AmpInUSDPos] &&
        r.c[basicBoost2AmpInUSDPos]?.v, //must always have something like col A
    };
  });
  return _airdropList;
}
function convertExcelResponseToAmpPayoutBoostedBonusList(
  excelRes: ExcelResponse
): AmpPayoutList {
  let _airdropList = {};
...
function convertExcelResponseToAirdropList(
  excelRes: ExcelResponse
): AirdropList {
  let _airdropList = {};
