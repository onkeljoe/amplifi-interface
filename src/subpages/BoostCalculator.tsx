    ...
    <>
      ...
      <Table>
        ...
        <tr className="basic_boosted">
          ...
          <td><div> 
          {/* <div>10% - 210% 🚀</div>  */}
            {projectedPayout && cre8rPrice && `Earn up to ${nFormatter(projectedPayout.debug[0].basicBoost / cre8rPrice * 600, 1)} $AMP ($${nFormatter(projectedPayout.debug[0].basicBribe * 1.1, 1)})`}</div>
          ...
          <td>
           ...
           <div>
             {projectedPayout && cre8rPrice && `${nFormatter(projectedPayout.debug[0].basicBoost2AmpInUSD / cre8rPrice * 600, 1)} $AMP ($${nFormatter(projectedPayout.debug[0].basicBoost2AmpInUSD, 1)})`} </div>
          </td>
        ...
        <tr className="boosted_bonus">
          ...
          <td>
            ...
            <td> 
              {/* 60% - 320% 🚀 */}
              <div>
                Earn up to {projectedPayout && cre8rPrice && `${nFormatter(amountUSDForBoostedBonus! / cre8rPrice * 600 * 5.714 / 2, 1)} $AMP`}
              </div>
              <div>
          ...
          <td>
             ...
             {projectedPayout && cre8rPrice && `${nFormatter(projectedPayout.debug[0].boostedBonus2AmpInUSD / cre8rPrice * 600, 1)} $AMP ($${nFormatter(projectedPayout.debug[0].boostedBonus2AmpInUSD, 1)})`}
          </td>
        ...
        <tr className="payout-row">
          ...
          <td></td>
          <td></td>
          <td>{projectedPayout && cre8rPrice && `${nFormatter(Math.max(projectedPayout.debug[0].basicBoost2AmpInUSD, projectedPayout.debug[0].boostedBonus2AmpInUSD) / cre8rPrice * 600, 1)} $AMP ($${nFormatter(Math.max(projectedPayout.debug[0].basicBoost2AmpInUSD, projectedPayout.debug[0].boostedBonus2AmpInUSD), 1)})`}
          </td>
          <td></td>
        </tr>
