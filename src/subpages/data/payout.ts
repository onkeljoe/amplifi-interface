...
export function calcPayouts(
  ...
  for (
    let i = 0;
    i < addresses.length && (limit == null ? true : i < limit);
    i += 1
  ) {
    const a = addresses[i];
    ...
    const basicBribeUSD =
      (voterToFBEETS[a] / totalFBeetsVotedForCRE8R) *
      100 *
      percentVoteForCre8r *
    ...
    if (dif <= -currentHoldings * 0.04) {
      //why do we need the &&? could we remove currentHoldings?
      bogusestBribe = basicBribeUSD * 0.5;
    } else {
      const hasLp3x =
        currentHoldings * (1 - MARGIN_OF_ERROR) > basicBribeUSD * 3;
      ...
      if (
        hasLp3x &&
        currentHoldings * 1.2 * (1 - MARGIN_OF_ERROR) >=
          lastHoldings + lastWeekPayoutInCRE8R
      ) {
        boostedBribe = basicBribeUSD * 1.25;
      }
      if (
        hasLp3x &&
        currentHoldings * (1 - MARGIN_OF_ERROR) >
          (lastWeekPayoutInCRE8R + lastHoldings) * 1.35
      ) {
        // currentHoldings > lastHoldings*1.35 + lastWeekPayout
      ...
      if (hasLp3x) {
        ...
        const multiplierHoldings =
          ((Math.min(2, ratioHoldings) - 1.35) * 1) / 1.65 / 2;
        // boostedBonus2 += multiplierHoldings * boostedBonus //extra bonus

        const excessCre8r = Math.max(
          0,
          currentHoldings - (lastHoldings + lastWeekPayoutInCRE8R) * 1.35
        );

        boostedBonus2 = Math.max(
          0,
          multiplierHoldings * excessCre8r * cre8rPrice
        );
      }
    }
