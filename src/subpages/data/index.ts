 ...
 * @returns
 */
function calculateLPTokenPriceInUSD(
  ...
  return token1Amount
    .mul(token1Price)
    .add(token2Amount.mul(token2Price))
    .div(circulatingSupply);
}

export async function getCRE8RPrice() {
  return fetch(
    "https://api.dexscreener.com/latest/dex/pairs/fantom/0xbb4607bede4610e80d35c15692efcb7807a2d0a6000200000000000000000140-0x21be370D5312f44cB42ce377BC9b8a0cEF1A4C83-0x2aD402655243203fcfa7dCB62F8A08cc2BA88ae0"
  )
    .then((res) => {
      if (res.status >= 400) {
...
export async function getFBeetsPrice() {
  return fetch(
    "https://api.dexscreener.com/latest/dex/pairs/fantom/0x2cea0da40cf133721377bb2b0bf4adc43715bfc30002000000000000000001e1-0x04068da6c83afcfa0e13ba15a6696662335d5b75-0xfcef8a994209d6916eb2c86cdd2afd60aa6f54b1"
  )
    .then((res) => {
      if (res.status >= 400) {
        throw new Error("Bad response from server");
...
export async function getFTMPrice() {
  return fetch(
    "https://min-api.cryptocompare.com/data/pricemultifull?fsyms=FTM&tsyms=USD"
  )
    .then((res) => {
      if (res.status >= 400) {
        throw new Error("Bad response from server");
...
export async function getSpiritLPCRE8R() {
  ...
  const ftmBalance = await getTokenBalance(
    provider,
    ERC20_ABI,
    ftmAddress,
    spiritLPCRE8rAddress
  );
  const cre8rBalance = await getTokenBalance(
    provider,
    ERC20_ABI,
    cre8rAddress,
    spiritLPCRE8rAddress
  );
  const circulatingSupply = await getTotalSupply(
    provider,
    ERC20_ABI,
    spiritLPCRE8rAddress
  );
  ...
  const spiritLPCRE8RPrice = calculateLPTokenPriceInUSD(
    circulatingSupply,
    ftmBalance,
    ftmPrice,
    cre8rBalance,
...
export async function getBeetsLPCRE8R() {
  ...
  if (data) {
    const lpCRE8RBeets =
      parseFloat(data.pool.totalLiquidity) / parseFloat(data.pool.totalShares);
    return lpCRE8RBeets;
  } else {
    console.error(error);
    return undefined;
