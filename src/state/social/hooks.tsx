...
const VERIFICATION_WORKER_URL =
  "https://sybil-silochad-production.cre8r.workers.dev";

// get all identity info from github file
export function useAllIdentities(): [
  Identities | undefined,
  (identities: Identities) => void
] {
  const dispatch = useDispatch<AppDispatch>();
  const identities = useSelector<AppState, AppState["social"]["identities"]>(
    (state) => state.social.identities
  );
...
export function useAllVerifiedHandles():
  | { [address: string]: TwitterEntry | undefined }
  | undefined {
...
export function useVerifyCallback(tweetID: string | undefined): {
  ...
  const verifyCallback = useCallback(() => {
    ...
    return fetch(
      `${VERIFICATION_WORKER_URL}/api/verify?account=${account}&id=${tweetID}`
    )
      .then(async (res) => {
        if (res.status === 200) {
          return {
...
export function useTwitterProfileData(
  handle: string | undefined | null
): TwitterProfileData | undefined {
...
export function useMultipleTwitterProfileDatas(
  ...
  const [formattedData, setFormattedData] =
    useState<{ [handle: string]: TwitterProfileData | undefined }>();
      ...
      Promise.all(
        ...
        .then((handlesData) => {
          setFormattedData(
            Object.assign(
              {},
              ...handlesData.map((key) => key && { [key.handle]: key })
            )
          );
        })
        .catch(() => undefined);
