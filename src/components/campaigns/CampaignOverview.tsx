...
const CampaignWrapper = styled.div<{ gridTemplateColumn?: string }>`
  ...
  grid-template-columns: ${({ gridTemplateColumn }) =>
    gridTemplateColumn ? gridTemplateColumn : "1 fr"};
  gap: 24px;
  width: 100%;
...
export default function CampaignOverview() {
  ...
  return (
    <Wrapper style={{ marginTop: 10 }}>
      ...
      <CampaignWrapper
        gridTemplateColumn={activeCampaign.overviewVideo ? "400px 1fr" : "1fr"}
      >
        <AutoColumn>
            ...
            <>
              {activeCampaign.budgetDescription && (
                <>
                  ...
                  <TYPE.body>{activeCampaign.budgetDescription}</TYPE.body>
                </>
              )}
              {activeCampaign.goal && (
                <>
